                            ...
                            if (smsinq != null)
                            {
                                ...
                                if (result < 0)// if (smsinq.dateofLimitet >= d) // Hanoz Ersal Shavad.
                                {
                                    ...
                                    if (user.Result.CreditFinance < 1)
                                    {
                                        ...
                                        //this as  a Dastan 
                                        //):
                                        var resultSend = _iSMSService.SMSFF(new SMSSendRequest { sender = "10000900900300", to = user.Result.Phone, txt = "کاربر گرامی حساب خود را شارژ کنید.  در غیر این صورت پیام های صف ارسالتان حذف خواهد شد" });
