...
{
    public class SendAnnualSMSWorker : BackgroundService
    {
       ...
       // private readonly ISMSService _iSMSService;
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                try
                {
                    await foreach (var item in _channelAnnual.ReturnValue(stoppingToken))
                    {
                        ...
                        if (smsinq != null)
                        {
                            ...
                            //this is better.
                            //3. update from Gueu
                            DateOnly newdat = new DateOnly(smsinq.dateOfsend.Year + 1, smsinq.dateOfsend.Month, smsinq.dateOfsend.Day);
                           var up= getQ.UpdateSMSinQbyIdF(smsinq.Id, newdat, smsinq.timeOfsend.TimeOfDay);
