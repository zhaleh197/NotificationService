...
namespace Notification.Application.Service.SMS.Queris.Get
{
    public class GetSMS : IGetSMS
    {
        ...
        {
            var res = _context.SMSClients.AsQueryable().ToList();

            var config = new MapperConfiguration(cfg =>
                cfg.CreateMap<List<SMSClient>, List<ResultGetClientSMS>>()
            );
        ...
        {
            var res = _context.SMSUsers.ToList().Select(us => new ResultGetUserSMS
            {
                Body = us.Body,
                Status = us.SMS_Resivers.Where(s=>s.IdSMS==us.Id).FirstOrDefault().SendStatus,
                DateSend = us.SMS_Resivers.Where(s => s.IdSMS == us.Id).FirstOrDefault().DateSend,
                DateDelivere = us.SMS_Resivers.Where(s => s.IdSMS == us.Id).FirstOrDefault().DateDelivere,
                Id = us.Id,
                IdUser = us.IdUser,
                Resiver = us.SMS_Resivers.Where(s=>s.IdSMS==us.Id).ToList().Select(s=>s.Resiver).ToList(),//14010413 // get all resiver from table SMS_Resivers
